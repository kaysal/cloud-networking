...
resource "aws_eip" "eu_w1a_vpc1_bastion" {
  instance = "${aws_instance.eu_w1a_vpc1_bastion.id}"
  vpc      = true
}
...
resource "aws_instance" "eu_w1a_vpc1_bastion" {
  ...
  tags {
    Name = "${var.name}eu-w1a-vpc1-bastion"
  }
}
