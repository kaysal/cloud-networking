...
resource "google_dns_record_set" "bastion" {
  ...
  name         = "bastion.ilb.${data.google_dns_managed_zone.cloudtuple_public.dns_name}"
  type         = "A"
  ttl          = 300
...
resource "google_dns_record_set" "bastion_eu_w1" {
  ...
  name         = "bastion.ilb.${data.google_dns_managed_zone.cloudtuple_private.dns_name}"
  type         = "A"
  ttl          = 300
