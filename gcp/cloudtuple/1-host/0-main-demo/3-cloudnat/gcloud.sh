...
gcloud beta compute routers create nat-gce-eu-w3-cr1 \
    --network $NETWORK \
    --region europe-west3
...
gcloud beta compute routers nats create gce-eu-w3-nat1 \
    --router-region europe-west3 \
    --router nat-gce-eu-w3-cr1 \
    --nat-custom-subnet-ip-ranges $GCE_EU_W3 \
    --auto-allocate-nat-external-ips \
    --min-ports-per-vm=64 \
    --udp-idle-timeout=60 \
    --icmp-idle-timeout=60 \
