...
resource "google_compute_vpn_tunnel" "to_aws_eu_w2_vpc1_cgw3_tunnel1" {
  ...
  peer_ip            = "${data.terraform_remote_state.aws_eu_west2_vpc1_data.aws_eu_w2_vpc1_cgw3_tunnel1_address}"
  ike_version = "1"
...
resource "google_compute_vpn_tunnel" "to_aws_eu_w2_vpc1_cgw3_tunnel2" {
  ...
  peer_ip            = "${data.terraform_remote_state.aws_eu_west2_vpc1_data.aws_eu_w2_vpc1_cgw3_tunnel2_address}"
  ike_version = "1"
  shared_secret      = "${var.preshared_key}"
  target_vpn_gateway = "${google_compute_vpn_gateway.us_e1_vpn_gw1.self_link}"
  router = "${google_compute_router.us_e1_cr1.name}"
