...
resource "google_compute_instance" "web_80" {
  name                      = "${local.prefix_spoke1}web-80"
  machine_type              = "n1-standard-1"
  zone                      = "${var.hub.trust1.region}-b"
  ...
  network_interface {
    ...
    network_ip = var.hub.trust1.ip.web80
    access_config {}
  }
...
resource "google_compute_instance" "web_81" {
  name                      = "${local.prefix_spoke1}web-81"
  machine_type              = "n1-standard-1"
  zone                      = "${var.hub.trust1.region}-b"
  metadata_startup_script   = local.web_init
  allow_stopping_for_update = "true"
  ...
  network_interface {
    subnetwork = local.subnet.self_link
    network_ip = var.hub.trust1.ip.web81
    access_config {}
  }
