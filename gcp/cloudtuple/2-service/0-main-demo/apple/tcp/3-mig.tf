...
resource "google_compute_region_instance_group_manager" "prod_eu_w2" {
  name               = "${var.name}prod-eu-w2"
  base_instance_name = "${var.name}prod-eu-w2"
  instance_template  = "${google_compute_instance_template.prod_template_eu_w2.self_link}"
  region       = "europe-west2"
  target_size = 1
