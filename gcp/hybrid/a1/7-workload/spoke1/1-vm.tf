...
locals {
  web_init = templatefile("scripts/web.sh.tpl", {
    ...
    SPOKE_8080 = var.spoke1.ip.web80
    SPOKE_8081 = var.spoke1.ip.web81
...
resource "google_compute_instance" "web_80" {
  name                      = "${var.global.prefix}${var.spoke1.prefix}web-80"
  ...
  network_interface {
    ...
    network_ip = var.spoke1.ip.web80
    access_config {}
  }

  service_account {
    scopes = ["cloud-platform"]
    email  = local.sa.spoke1.email
  }
}
...
resource "google_compute_instance" "web_81" {
  name                      = "${var.global.prefix}${var.spoke1.prefix}web-81"
  machine_type              = "n1-standard-1"
  zone                      = "${var.spoke1.region}-b"
  metadata_startup_script   = local.web_init
  allow_stopping_for_update = "true"
  ...
  network_interface {
    subnetwork = local.subnet.self_link
    network_ip = var.spoke1.ip.web81
    access_config {}
  }
