...
# Create a private subnet to launch instance to
...
# Grant the VPC access to the VPN gateway
# on its main route table
...
# define a static route between a VPN connection
# and a customer gateway create a route to GCP
# loadbalancer subnet
...
resource "aws_security_group" "default" {
  ...
  # ICMP access from remote GCP network subnet
  ingress {
    from_port   = 8
    to_port     = 0
    protocol    = "icmp"
    cidr_blocks = ["${var.vpn_dst_cidr_block}"]
  }
}
