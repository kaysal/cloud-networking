...
# LAUNCH TEMPLATE SG
#==============================
resource "aws_security_group" "launch_templ_prv_sg" {
  name   = "${var.name}launch-templ-prv-sg"
  vpc_id = "${aws_vpc.vpc1.id}"
...
resource "aws_security_group_rule" "launch_templ_http_ingress" {
  type                     = "ingress"
  from_port                = 80
  to_port                  = 80
  protocol                 = "tcp"
  source_security_group_id = "${aws_security_group.alb_pub_sg.id}"
  security_group_id        = "${aws_security_group.launch_templ_prv_sg.id}"
}

resource "aws_security_group_rule" "launch_templ_bastion_ingress" {
  type                     = "ingress"
  from_port                = 0
  to_port                  = 0
  protocol                 = "-1"
...
resource "aws_security_group_rule" "launch_templ_egress" {
  type              = "egress"
  from_port         = 0
  to_port           = 0
  protocol          = "-1"
...
output "launch_templ_prv_sg" {
  value = "${aws_security_group.launch_templ_prv_sg.id}"
}
