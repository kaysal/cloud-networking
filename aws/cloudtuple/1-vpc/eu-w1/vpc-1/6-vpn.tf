...
resource "aws_customer_gateway" "vpc1_cgw1" {
  ...
  ip_address = "${data.terraform_remote_state.vpc.gcp_eu_w1_vpn_gw1_ip}"
  type       = "ipsec.1"
...
resource "aws_customer_gateway" "vpc1_cgw2" {
  bgp_asn    = "${var.customer_side_asn}"
  ip_address = "${data.terraform_remote_state.vpc.gcp_eu_w1_vpn_gw2_ip}"
  type       = "ipsec.1"
