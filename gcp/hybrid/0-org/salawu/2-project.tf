...
resource "google_project" "onprem_project" {
  name            = var.onprem_project
  project_id      = var.onprem_project
  folder_id       = google_folder.onprem.name
...
resource "google_project_iam_member" "project_iam_admin_vm_spoke1" {
  project = google_project.spoke1_project.name
...
resource "google_project_iam_member" "project_iam_admin_vm_spoke2" {
  project = google_project.spoke2_project.name
  role    = "roles/resourcemanager.projectIamAdmin"
  member  = "serviceAccount:${google_service_account.vm_spoke2_project.email}"
}
