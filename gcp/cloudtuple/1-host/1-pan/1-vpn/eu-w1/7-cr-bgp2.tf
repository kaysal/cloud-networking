...
resource "google_compute_router_interface" "aws_eu_w1_vpc1_tunnel2" {
  ...
  ip_range   = "${data.terraform_remote_state.aws_eu_w1_vpc1.vyosb_gcp_tunnel_inside_address2}"
  region     = "europe-west1"
}
...
resource "google_compute_router_peer" "aws_eu_w1_vpc1_tunnel2" {
  name                      = "${var.name}aws-eu-w1-vpc1-tunnel2"
  router                    = "${google_compute_router.eu_w1_cr_vpn.name}"
  peer_ip_address           = "${data.terraform_remote_state.aws_eu_w1_vpc1.vyosb_aws_tunnel_inside_address2}"
  peer_asn                  = 65010
