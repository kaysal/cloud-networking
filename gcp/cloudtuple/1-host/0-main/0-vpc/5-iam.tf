...
resource "google_project_iam_binding" "project_network_user" {
  ...
  members = [
    ...
    "serviceAccount:${data.terraform_remote_state.apple.outputs.apple_service_project_number}@cloudservices.gserviceaccount.com",
    "serviceAccount:service-${data.terraform_remote_state.host.outputs.host_project_number}@gcf-admin-robot.iam.gserviceaccount.com",
  ]
}
...
resource "google_compute_subnetwork_iam_binding" "gke_eu_w1_10_0_4" {
  ...
  members = [
    "group:gke-grp@cloudtuple.com",
    "serviceAccount:${data.terraform_remote_state.gke.outputs.gke_service_project_number}@cloudservices.gserviceaccount.com",
    "serviceAccount:service-${data.terraform_remote_state.gke.outputs.gke_service_project_number}@container-engine-robot.iam.gserviceaccount.com",
  ]
}
...
resource "google_project_iam_binding" "host_prokect_viewer" {
  project = data.terraform_remote_state.host.outputs.host_project_id
  role    = "roles/viewer"
