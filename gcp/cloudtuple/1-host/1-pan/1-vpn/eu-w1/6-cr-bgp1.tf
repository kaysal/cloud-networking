...
resource "google_compute_router_interface" "aws_eu_w1_vpc1_tunnel1" {
  ...
  ip_range   = "${data.terraform_remote_state.aws_eu_w1_vpc1.vyosa_gcp_tunnel_inside_address2}"
  region     = "europe-west1"
}
...
resource "google_compute_router_peer" "aws_eu_w1_vpc1_tunnel1" {
  name                      = "${var.name}aws-eu-w1-vpc1-tunnel1"
  router                    = "${google_compute_router.eu_w1_cr_vpn.name}"
  peer_ip_address           = "${data.terraform_remote_state.aws_eu_w1_vpc1.vyosa_aws_tunnel_inside_address2}"
  peer_asn                  = 65010
